...
func (q *DBQuerier) FindUsers(ctx context.Context, limit int, offset int) ([]FindUsersRow, error) {
	ctx = context.WithValue(ctx, "pggen_query_name", "FindUsers")
	rows, err := q.conn.Query(ctx, findUsersSQL, limit, offset)
	if err != nil {
...
func (q *DBQuerier) FindUsersBatch(batch genericBatch, limit int, offset int) {
	batch.Queue(findUsersSQL, limit, offset)
}
