...
CREATE TABLE IF NOT EXISTS phase_status_timestamps (
    ...
    CONSTRAINT phase_status_timestamps_run_id_fkey FOREIGN KEY (run_id) REFERENCES plans(run_id) ON UPDATE CASCADE ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS repohooks (
    ...
    CONSTRAINT repo_connections_module_id_fkey FOREIGN KEY (module_id) REFERENCES modules(module_id) ON UPDATE CASCADE ON DELETE CASCADE,
    CONSTRAINT repo_connections_workspace_id_fkey FOREIGN KEY (workspace_id) REFERENCES workspaces(workspace_id) ON UPDATE CASCADE ON DELETE CASCADE,
    CONSTRAINT vcs_provider_id_fk FOREIGN KEY (vcs_provider_id) REFERENCES vcs_providers(vcs_provider_id) ON UPDATE CASCADE ON DELETE CASCADE
);
...
CREATE TABLE IF NOT EXISTS workspaces (
    ...
    CONSTRAINT current_state_version_id_fk FOREIGN KEY (current_state_version_id) REFERENCES state_versions(state_version_id) ON UPDATE CASCADE,
    CONSTRAINT latest_run_id_fk FOREIGN KEY (latest_run_id) REFERENCES runs(run_id) ON UPDATE CASCADE,
    CONSTRAINT lock_run_id_fk FOREIGN KEY (lock_run_id) REFERENCES runs(run_id) ON UPDATE CASCADE,
...
CREATE OR REPLACE TRIGGER delete_tags AFTER DELETE ON workspace_tags FOR EACH STATEMENT EXECUTE FUNCTION public.delete_tags();

CREATE OR REPLACE TRIGGER notify_event AFTER INSERT OR DELETE OR UPDATE ON agent_pools FOR EACH ROW EXECUTE FUNCTION public.agent_pools_notify_event();

CREATE OR REPLACE TRIGGER notify_event AFTER INSERT OR DELETE OR UPDATE ON agents FOR EACH ROW EXECUTE FUNCTION public.agents_notify_event();

CREATE OR REPLACE TRIGGER notify_event AFTER INSERT OR DELETE OR UPDATE ON jobs FOR EACH ROW EXECUTE FUNCTION public.jobs_notify_event();

CREATE OR REPLACE TRIGGER notify_event AFTER INSERT OR DELETE OR UPDATE ON logs FOR EACH ROW EXECUTE FUNCTION public.logs_notify_event();

CREATE OR REPLACE TRIGGER notify_event AFTER INSERT OR DELETE OR UPDATE ON notification_configurations FOR EACH ROW EXECUTE FUNCTION public.notification_configurations_notify_event();

CREATE OR REPLACE TRIGGER notify_event AFTER INSERT OR DELETE OR UPDATE ON organizations FOR EACH ROW EXECUTE FUNCTION public.organizations_notify_event();

CREATE OR REPLACE TRIGGER notify_event AFTER INSERT OR DELETE OR UPDATE ON runs FOR EACH ROW EXECUTE FUNCTION public.runs_notify_event();

CREATE OR REPLACE TRIGGER notify_event AFTER INSERT OR DELETE OR UPDATE ON workspaces FOR EACH ROW EXECUTE FUNCTION public.workspaces_notify_event();
