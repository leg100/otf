...
func (s *Server) CreateRun(w http.ResponseWriter, r *http.Request) {
	...
	run, err := s.RunService().CreateRun(r.Context(), workspace, otf.RunCreateOptions{
...
func (s *Server) GetRun(w http.ResponseWriter, r *http.Request) {
	vars := mux.Vars(r)
	run, err := s.RunService().GetRun(r.Context(), vars["id"])
	if err != nil {
		writeError(w, http.StatusNotFound, err)
		return
	}
...
func (s *Server) listRuns(w http.ResponseWriter, r *http.Request, opts otf.RunListOptions) {
	...
	rl, err := s.RunService().ListRun(r.Context(), opts)
	if err != nil {
		writeError(w, http.StatusNotFound, err)
		return
	}
	if opts.Include != nil {
		for _, include := range strings.Split(*opts.Include, ",") {
			if include == "workspace" {
				ws, err := s.WorkspaceService().GetWorkspace(r.Context(), otf.WorkspaceSpec{
					ID: opts.WorkspaceID,
				})
				if err != nil {
					writeError(w, http.StatusNotFound, err)
...
func (s *Server) ApplyRun(w http.ResponseWriter, r *http.Request) {
	...
	if err := s.RunService().ApplyRun(r.Context(), vars["id"], opts); err != nil {
		writeError(w, http.StatusNotFound, err)
		return
	}
...
func (s *Server) DiscardRun(w http.ResponseWriter, r *http.Request) {
	...
	err := s.RunService().DiscardRun(r.Context(), vars["id"], opts)
...
func (s *Server) CancelRun(w http.ResponseWriter, r *http.Request) {
	...
	err := s.RunService().CancelRun(r.Context(), vars["id"], opts)
...
func (s *Server) ForceCancelRun(w http.ResponseWriter, r *http.Request) {
	...
	err := s.RunService().ForceCancelRun(r.Context(), vars["id"], opts)
