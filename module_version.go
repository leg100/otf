...
// Upload validates and uploads a tarball for a module version. Statuses are set
// accordingly for both the module version and its parent module.
func (u *ModuleVersionUploader) Upload(ctx context.Context, opts UploadModuleVersionOptions) (*Module, *ModuleVersion, error) {
	...
	_, validErr := UnmarshalTerraformModule(opts.Tarball)
	if validErr != nil {
		var err error
		mod, modver, err = UpdateModuleVersionStatus(ctx, u.Application, UpdateModuleVersionStatusOptions{
	...
	err := u.Tx(ctx, func(tx Application) (err error) {
		mod, modver, err = UpdateModuleVersionStatus(ctx, tx, UpdateModuleVersionStatusOptions{
			ID:     opts.ModuleVersionID,
		...
		return tx.DB().UploadModuleVersion(ctx, opts)
	})
	return mod, modver, err
}
...
func UpdateModuleVersionStatus(ctx context.Context, app Application, opts UpdateModuleVersionStatusOptions) (*Module, *ModuleVersion, error) {
	...
	err := app.Tx(ctx, func(tx Application) (err error) {
