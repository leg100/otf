...
func NewApplication(logger logr.Logger, db otf.DB, cache *bigcache.BigCache) (*Application, error) {
	...
	runCreator := NewRunCreator(db.RunStore(), logger, eventService)
	configurationVersionService := NewConfigurationVersionService(db.ConfigurationVersionStore(), logger, runCreator, workspaceService, cache)
	runService := NewRunService(db.RunStore(), logger, runCreator, workspaceService, configurationVersionService, eventService, db.PlanLogStore(), db.ApplyLogStore(), cache)
