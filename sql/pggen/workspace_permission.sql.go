...
const findWorkspacePermissionsByRunIDSQL = `SELECT
    p.role,
    (t.*)::"teams" AS team,
    (o.*)::"organizations" AS organization
FROM workspace_permissions p
...
type FindWorkspacePermissionsByRunIDRow struct {
	Role         pgtype.Text    `json:"role"`
	Team         *Teams         `json:"team"`
	Organization *Organizations `json:"organization"`
}

// FindWorkspacePermissionsByRunID implements Querier.FindWorkspacePermissionsByRunID.
func (q *DBQuerier) FindWorkspacePermissionsByRunID(ctx context.Context, runID pgtype.Text) ([]FindWorkspacePermissionsByRunIDRow, error) {
	ctx = context.WithValue(ctx, "pggen_query_name", "FindWorkspacePermissionsByRunID")
	rows, err := q.conn.Query(ctx, findWorkspacePermissionsByRunIDSQL, runID)
	if err != nil {
		return nil, fmt.Errorf("query FindWorkspacePermissionsByRunID: %w", err)
	}
	defer rows.Close()
	items := []FindWorkspacePermissionsByRunIDRow{}
	teamRow := q.types.newTeams()
	organizationRow := q.types.newOrganizations()
	for rows.Next() {
		var item FindWorkspacePermissionsByRunIDRow
		if err := rows.Scan(&item.Role, teamRow, organizationRow); err != nil {
			return nil, fmt.Errorf("scan FindWorkspacePermissionsByRunID row: %w", err)
		}
		if err := teamRow.AssignTo(&item.Team); err != nil {
			return nil, fmt.Errorf("assign FindWorkspacePermissionsByRunID row: %w", err)
		}
		if err := organizationRow.AssignTo(&item.Organization); err != nil {
			return nil, fmt.Errorf("assign FindWorkspacePermissionsByRunID row: %w", err)
		}
	...
	if err := rows.Err(); err != nil {
		return nil, fmt.Errorf("close FindWorkspacePermissionsByRunID rows: %w", err)
	}
	return items, err
}

// FindWorkspacePermissionsByRunIDBatch implements Querier.FindWorkspacePermissionsByRunIDBatch.
func (q *DBQuerier) FindWorkspacePermissionsByRunIDBatch(batch genericBatch, runID pgtype.Text) {
	batch.Queue(findWorkspacePermissionsByRunIDSQL, runID)
}

// FindWorkspacePermissionsByRunIDScan implements Querier.FindWorkspacePermissionsByRunIDScan.
func (q *DBQuerier) FindWorkspacePermissionsByRunIDScan(results pgx.BatchResults) ([]FindWorkspacePermissionsByRunIDRow, error) {
	rows, err := results.Query()
	if err != nil {
		return nil, fmt.Errorf("query FindWorkspacePermissionsByRunIDBatch: %w", err)
	}
	defer rows.Close()
	items := []FindWorkspacePermissionsByRunIDRow{}
	teamRow := q.types.newTeams()
	...
	for rows.Next() {
		var item FindWorkspacePermissionsByRunIDRow
		if err := rows.Scan(&item.Role, teamRow, organizationRow); err != nil {
			return nil, fmt.Errorf("scan FindWorkspacePermissionsByRunIDBatch row: %w", err)
		}
		if err := teamRow.AssignTo(&item.Team); err != nil {
			return nil, fmt.Errorf("assign FindWorkspacePermissionsByRunID row: %w", err)
		}
	...
	if err := rows.Err(); err != nil {
		return nil, fmt.Errorf("close FindWorkspacePermissionsByRunIDBatch rows: %w", err)
	}
	return items, err
}
