...
func deleteBackendConfigFromDirectory(ctx context.Context, exe *Execution) error {
	return filepath.Walk(exe.Path, func(path string, info os.FileInfo, err error) error {
		if err != nil {
			return nil
		}

		if exe.Path != path && info.IsDir() {
