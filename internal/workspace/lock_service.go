...
func (s *Service) Lock(ctx context.Context, workspaceID resource.TfeID, runID *resource.TfeID) (*Workspace, error) {
	...
	if runID != nil {
		...
		subject, err := s.Authorize(ctx, resource.Lock, resource.WorkspaceKind, workspaceID)
		if err != nil {
			return nil, err
		}
...
func (s *Service) Unlock(ctx context.Context, workspaceID resource.TfeID, runID *resource.TfeID, force bool) (*Workspace, error) {
	...
	if runID != nil {
		...
		if force {
			action = resource.ForceUnlock
		} else {
			action = resource.Unlock
		}
		subject, err := s.Authorize(ctx, action, resource.WorkspaceKind, workspaceID)
		if err != nil {
			return nil, err
		}
		user, ok := subject.(*user.User)
