...
func (h *WebsocketListHandler[Resource, Options]) handler(w http.ResponseWriter, r *http.Request, opts Options) {
	...
	g.Go(func() error {
		...
		// second.
		var valve <-chan time.Time
		for range sub {
			<-valve
			if err := sendList(); err != nil {
				return err
			}
			valve = time.After(time.Second)
		}
		return nil
	})
	g.Go(func() error {
