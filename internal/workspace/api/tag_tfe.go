...
func (a *tfe) listTags(w http.ResponseWriter, r *http.Request) {
	...
	// convert to
	to := make([]*workspace.TFEOrganizationTag, len(page.Items))
...
func (a *tfe) alterWorkspaceTags(w http.ResponseWriter, r *http.Request, op tagOperation) {
	...
	specs := make([]workspace.TagSpec, len(params))
	for i, tag := range params {
		specs[i] = workspace.TagSpec{ID: tag.ID, Name: tag.Name}
	}
...
func (a *tfe) toTag(from *workspace.Tag) *workspace.TFEOrganizationTag {
	return &workspace.TFEOrganizationTag{
		ID:            from.ID,
		Name:          from.Name,
