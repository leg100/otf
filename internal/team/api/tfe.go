...
type tfeClient interface {
	...
	GetTeam(ctx context.Context, organization organization.Name, name string) (*team.Team, error)
	...
	CreateTeamToken(ctx context.Context, opts team.CreateTokenOptions) (*team.Token, []byte, error)
	GetTeamToken(ctx context.Context, teamID resource.TfeID) (*team.Token, error)
...
func (a *TFEAPI) createTeam(w http.ResponseWriter, r *http.Request) {
	...
	var params team.TFETeamCreateOptions
	if err := tfeapi.Unmarshal(r.Body, &params); err != nil {
		tfeapi.Error(w, err)
		return
	}

	opts := team.CreateTeamOptions{
		Name:       params.Name,
...
func (a *TFEAPI) updateTeam(w http.ResponseWriter, r *http.Request) {
	...
	var params team.TFETeamUpdateOptions
	if err := tfeapi.Unmarshal(r.Body, &params); err != nil {
		tfeapi.Error(w, err)
		return
	}

	opts := team.UpdateTeamOptions{
		Name:       params.Name,
		SSOTeamID:  params.SSOTeamID,
		Visibility: params.Visibility,
	}
...
func (a *TFEAPI) listTeams(w http.ResponseWriter, r *http.Request) {
	...
	// convert items
	items := make([]*team.TFETeam, len(teams))
...
func (a *TFEAPI) convertTeam(from *team.Team) *team.TFETeam {
	return &team.TFETeam{
		ID:         from.ID,
		Name:       from.Name,
		...
		Permissions: &team.TFETeamPermissions{
			CanDestroy:          true,
...
func (a *TFEAPI) createTeamToken(w http.ResponseWriter, r *http.Request) {
	...
	ot, token, err := a.Client.CreateTeamToken(r.Context(), team.CreateTokenOptions{
		TeamID: id,
