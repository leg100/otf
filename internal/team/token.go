...
func (a *Service) CreateTeamToken(ctx context.Context, opts CreateTokenOptions) (*Token, []byte, error) {
	_, err := a.CanAccess(ctx, rbac.CreateTeamTokenAction, &authz.AccessRequest{ID: &opts.TeamID})
	if err != nil {
		return nil, nil, err
	}
...
func (a *Service) GetTeamToken(ctx context.Context, teamID resource.ID) (*Token, error) {
	_, err := a.CanAccess(ctx, rbac.GetTeamTokenAction, &authz.AccessRequest{ID: &teamID})
	if err != nil {
		return nil, err
	}
...
func (a *Service) DeleteTeamToken(ctx context.Context, teamID resource.ID) error {
	_, err := a.CanAccess(ctx, rbac.DeleteTeamTokenAction, &authz.AccessRequest{ID: &teamID})
	if err != nil {
		return err
	}
