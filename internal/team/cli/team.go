...
type teamClient interface {
	...
	GetTeam(ctx context.Context, organization organization.Name, team string) (*team.Team, error)
...
func NewTeamCommand(apiClient *otfhttp.Client) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		PersistentPreRunE: func(cmd *cobra.Command, args []string) error {
			...
			cli.client = &teamapi.Client{Client: apiClient}
			return nil
		},
	}
...
func (a *teamCLI) teamNewCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			team, err := a.client.CreateTeam(cmd.Context(), orgName, team.CreateTeamOptions{
				Name: new(args[0]),
			})
			if err != nil {
				return err
