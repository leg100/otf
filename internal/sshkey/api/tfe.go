...
type tfeClient interface {
	...
	UpdateSSHKey(ctx context.Context, id resource.TfeID, opts sshkey.UpdateOptions) (*sshkey.SSHKey, error)
...
func (a *TFEAPI) createSSHKey(w http.ResponseWriter, r *http.Request) {
	...
	key, err := a.Client.CreateSSHKey(r.Context(), sshkey.CreateOptions{
...
func (a *TFEAPI) listSSHKeys(w http.ResponseWriter, r *http.Request) {
	...
	to := make([]*sshkey.TFESSHKey, len(keys))
	for i, k := range keys {
		to[i] = a.convert(k)
	}
...
func (a *TFEAPI) updateSSHKey(w http.ResponseWriter, r *http.Request) {
	...
	key, err := a.Client.UpdateSSHKey(r.Context(), id, sshkey.UpdateOptions{
		Name: params.Name,
	})
	if err != nil {
		tfeapi.Error(w, err)
...
func (a *TFEAPI) convert(from *sshkey.SSHKey) *sshkey.TFESSHKey {
	return &sshkey.TFESSHKey{
		ID:   from.ID,
		Name: from.Name,
	}
}
