...
func (db *pgdb) listTeamUsers(ctx context.Context, teamID resource.ID) ([]*User, error) {
	result, err := db.Querier(ctx).FindUsersByTeamID(ctx, sql.ID(teamID))
	if err != nil {
		return nil, err
	}
...
func (db *pgdb) getUser(ctx context.Context, spec UserSpec) (*User, error) {
	if spec.UserID != nil {
		result, err := db.Querier(ctx).FindUserByID(ctx, sql.ID(*spec.UserID))
		if err != nil {
			return nil, err
		}
		...
		result, err := db.Querier(ctx).FindUserByAuthenticationTokenID(ctx, sql.ID(*spec.AuthenticationTokenID))
		if err != nil {
			return nil, sql.Error(err)
		}
		return dbresult(result).toUser(), nil
...
func (db *pgdb) DeleteUser(ctx context.Context, spec UserSpec) error {
	if spec.UserID != nil {
		_, err := db.Querier(ctx).DeleteUserByID(ctx, sql.ID(*spec.UserID))
		if err != nil {
			return sql.Error(err)
		}
	} else if spec.Username != nil {
...
func (db *pgdb) getUserToken(ctx context.Context, id resource.ID) (*UserToken, error) {
	row, err := db.Querier(ctx).FindTokenByID(ctx, sql.ID(id))
	if err != nil {
		return nil, sql.Error(err)
	}
	return &UserToken{
...
func (db *pgdb) deleteUserToken(ctx context.Context, id resource.ID) error {
	_, err := db.Querier(ctx).DeleteTokenByID(ctx, sql.ID(id))
	if err != nil {
		return sql.Error(err)
	}
	return nil
