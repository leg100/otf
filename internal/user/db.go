...
func (db *pgdb) addTeamMembership(ctx context.Context, teamID resource.ID, usernames ...Username) error {
	_, err := db.Exec(ctx, `
WITH
    users AS (
        SELECT username
...
func (db *pgdb) removeTeamMembership(ctx context.Context, teamID resource.ID, usernames ...Username) error {
	_, err := db.Exec(ctx, `
WITH
    users AS (
        SELECT username
