...
func TestCLI_State(t *testing.T) {
		...
		tests := []struct {
		...
		}{
			{
				...
				newFakeCLI(
					...
					withStateVersion(&state.Version{ID: testutils.ParseID(t, "sv-3"), WorkspaceID: testutils.ParseID(t, "ws-123")}),
	...
	t.Run("rollback", func(t *testing.T) {
		sv := &state.Version{ID: testutils.ParseID(t, "sv-456")}
...
func withStateVersion(sv *state.Version) fakeCLIOption {
	return func(c *fakeCLIService) {
		c.stateVersion = sv
	}
}

func withStateVersionList(svl *resource.Page[*state.Version]) fakeCLIOption {
	return func(c *fakeCLIService) {
...
func (f *fakeCLIService) ListStateVersions(context.Context, resource.TfeID, resource.PageOptions) (*resource.Page[*state.Version], error) {
	return f.stateVersionList, nil
}

func (f *fakeCLIService) GetCurrentStateVersion(ctx context.Context, workspaceID resource.TfeID) (*state.Version, error) {
...
func (f *fakeCLIService) DeleteStateVersion(ctx context.Context, svID resource.TfeID) error {
	return nil
}

func (f *fakeCLIService) RollbackStateVersion(ctx context.Context, svID resource.TfeID) (*state.Version, error) {
	return f.stateVersion, nil
}

func (f *fakeCLIService) DownloadState(ctx context.Context, svID resource.TfeID) ([]byte, error) {
	return f.state, nil
}

func (f *fakeCLIService) GetWorkspaceByName(context.Context, organization.Name, string) (*workspace.Workspace, error) {
