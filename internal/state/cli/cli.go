...
type cliStateService interface {
	...
	RollbackStateVersion(ctx context.Context, versionID resource.TfeID) (*state.Version, error)
...
func NewCommand(client *otfhttp.Client) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		PersistentPreRunE: func(cmd *cobra.Command, args []string) error {
			...
			cli.state = &stateapi.Client{Client: client}
			cli.workspaces = &workspaceapi.Client{Client: client}
			return nil
		},
	}
...
func (a *CLI) stateDeleteCommand() *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err := a.state.DeleteStateVersion(cmd.Context(), id); err != nil {
				return err
			}
...
func (a *CLI) stateDownloadCommand() *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			state, err := a.state.DownloadState(cmd.Context(), id)
			if err != nil {
				return err
			}
...
func (a *CLI) stateRollbackCommand() *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			_, err = a.state.RollbackStateVersion(cmd.Context(), id)
			if err != nil {
				return err
			}
