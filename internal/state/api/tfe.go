...
type tfeClient interface {
	...
	GetStateOutput(ctx context.Context, outputID resource.TfeID) (*state.Output, error)
}
...
func (a *tfe) createVersion(w http.ResponseWriter, r *http.Request) {
	...
	opts := state.TFEStateVersionCreateVersionOptions{}
	...
	// decodedState is optional as of terraform v1.6.0
	...
	if opts.State != nil {
		...
		if fmt.Sprintf("%x", md5.Sum(decodedState)) != *opts.MD5 {
			tfeapi.Error(w, err)
			return
		}
	}
	...
	sv, err := a.client.CreateStateVersion(r.Context(), state.CreateStateVersionOptions{
...
func (a *tfe) listVersionsByName(w http.ResponseWriter, r *http.Request) {
	...
	page, err := a.client.ListStateVersions(r.Context(), ws.ID, resource.PageOptions(opts.PageOptions))
	if err != nil {
		tfeapi.Error(w, err)
		return
	}
...
func (a *tfe) rollbackVersion(w http.ResponseWriter, r *http.Request) {
	opts := state.TFERollbackStateVersionOptions{}
	if err := tfeapi.Unmarshal(r.Body, &opts); err != nil {
		tfeapi.Error(w, err)
		return
	}
...
func (a *tfe) getCurrentVersionOutputs(w http.ResponseWriter, r *http.Request) {
	...
	to := make([]*state.TFEStateVersionOutput, len(sv.Outputs))
...
func (a *tfe) listOutputs(w http.ResponseWriter, r *http.Request) {
	...
	page := resource.NewPage(maps.Values(sv.Outputs), resource.PageOptions(params.PageOptions), nil)

	// convert to list of tfe types
	items := make([]*state.TFEStateVersionOutput, len(page.Items))
...
func (a *tfe) toStateVersion(from *state.Version, r *http.Request) (*state.TFEStateVersion, error) {
	to := &state.TFEStateVersion{
		ID:                 from.ID,
		CreatedAt:          from.CreatedAt,
		Serial:             from.Serial,
		Status:             state.TFEStateVersionStatus(from.Status),
		DownloadURL:        fmt.Sprintf("/api/v2/state-versions/%s/download", from.ID),
		ResourcesProcessed: true,
		Outputs:            make([]*state.TFEStateVersionOutput, len(from.Outputs)),
	}
	...
	for i, out := range maps.Values(from.Outputs) {
		to.Outputs[i] = &state.TFEStateVersionOutput{ID: out.ID}
	}
	if from.State != nil {
...
func (a *tfe) toStateVersionList(from *resource.Page[*state.Version], r *http.Request) ([]*state.TFEStateVersion, error) {
	// convert items
	items := make([]*state.TFEStateVersion, len(from.Items))
...
func (*tfe) toOutput(from *state.Output, scrubSensitive bool) *state.TFEStateVersionOutput {
	to := &state.TFEStateVersionOutput{
		ID:        from.ID,
		Name:      from.Name,
