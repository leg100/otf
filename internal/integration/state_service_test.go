...
func TestIntegration_StateService(t *testing.T) {
	...
	t.Run("create", func(t *testing.T) {
		...
		_, err = svc.State.CreateStateVersion(ctx, state.CreateStateVersionOptions{
			State:       file,
	...
	t.Run("get", func(t *testing.T) {
		...
		got, err := svc.State.GetStateVersion(ctx, want.ID)
		require.NoError(t, err)
	...
	t.Run("get not found error", func(t *testing.T) {
		...
		_, err := svc.State.GetStateVersion(ctx, "sv-99999")
	...
	t.Run("get current", func(t *testing.T) {
		...
		got, err := svc.State.GetCurrentStateVersion(ctx, want.WorkspaceID)
		require.NoError(t, err)
	...
	t.Run("get current not found error", func(t *testing.T) {
		...
		_, err := svc.State.GetCurrentStateVersion(ctx, "ws-99999")
	...
	t.Run("list", func(t *testing.T) {
		...
		got, err := svc.State.ListStateVersions(ctx, ws.ID, resource.PageOptions{})
		require.NoError(t, err)
	...
	t.Run("list not found error", func(t *testing.T) {
		...
		_, err := svc.State.ListStateVersions(ctx, "ws-does-not-exist", resource.PageOptions{})
		assert.Equal(t, internal.ErrResourceNotFound, err)
	})
	...
	t.Run("delete", func(t *testing.T) {
		...
		err := svc.State.DeleteStateVersion(ctx, want.ID)
		require.NoError(t, err)

		_, err = svc.State.GetStateVersion(ctx, want.ID)
		assert.Equal(t, internal.ErrResourceNotFound, err)

		t.Run("deleting current version not allowed", func(t *testing.T) {
			err := svc.State.DeleteStateVersion(ctx, current.ID)
