...
func TestIntegration_Agents(t *testing.T) {
	...
	pool1, err := daemon.Runners.CreateAgentPool(ctx, agentpkg.CreateAgentPoolOptions{
		Name:         "pool-1",
		Organization: org.Name,
	})
	require.NoError(t, err)

	pool2, err := daemon.Runners.CreateAgentPool(ctx, agentpkg.CreateAgentPoolOptions{
		Name:         "pool-2",
		Organization: org.Name,
	})
	require.NoError(t, err)
