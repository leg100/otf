...
func TestTx(t *testing.T) {
	...
	err = db.Tx(ctx, func(txCtx context.Context, q *sqlc.Queries) error {
		...
		// same tx
		_, err = q.FindOrganizationByID(txCtx, sql.String(org.ID.String()))
		assert.NoError(t, err)

		// this should succeed because it is using the same ctx from the same tx
		_, err = db.Querier(txCtx).FindOrganizationByID(txCtx, sql.String(org.ID.String()))
		assert.NoError(t, err)

		err = db.Tx(txCtx, func(ctx context.Context, q *sqlc.Queries) error {
			// this should succeed because it is using a child tx via the
			// querier
			_, err = q.FindOrganizationByID(ctx, sql.String(org.ID.String()))
			assert.NoError(t, err)

			// this should succeed because it is using a child tx via the
			// context
			_, err = db.Querier(ctx).FindOrganizationByID(ctx, sql.String(org.ID.String()))
			assert.NoError(t, err)
		...
		_, err = db.Querier(ctx).FindOrganizationByID(txCtx, sql.String(org.ID.String()))
