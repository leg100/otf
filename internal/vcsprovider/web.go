...
func (h *webHandlers) new(w http.ResponseWriter, r *http.Request) {
	...
	plugin, ok := h.registry[params.Kind]
	if !ok {
		html.Error(w, fmt.Sprintf("vcs provider kind %s not found", params.Kind), http.StatusUnprocessableEntity)
		return
	}
	plugin.NewHandler(w, r)
