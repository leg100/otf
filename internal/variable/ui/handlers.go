...
type Client interface {
	CreateWorkspaceVariable(ctx context.Context, workspaceID resource.ID, opts variable.CreateVariableOptions) (*variable.Variable, error)
	ListWorkspaceVariables(ctx context.Context, workspaceID resource.ID) ([]*variable.Variable, error)
	GetWorkspaceVariable(ctx context.Context, variableID resource.ID) (*variable.WorkspaceVariable, error)
	UpdateWorkspaceVariable(ctx context.Context, variableID resource.ID, opts variable.UpdateVariableOptions) (*variable.WorkspaceVariable, error)
	DeleteWorkspaceVariable(ctx context.Context, variableID resource.ID) (*variable.WorkspaceVariable, error)
	ListWorkspaceVariableSets(ctx context.Context, workspaceID resource.ID) ([]*variable.VariableSet, error)
	ListVariableSets(ctx context.Context, organization organization.Name) ([]*variable.VariableSet, error)
	GetVariableSet(ctx context.Context, setID resource.ID) (*variable.VariableSet, error)
	...
	UpdateVariableSet(ctx context.Context, setID resource.ID, opts variable.UpdateVariableSetOptions) (*variable.VariableSet, error)
	DeleteVariableSet(ctx context.Context, setID resource.ID) (*variable.VariableSet, error)
	GetVariableSetByVariableID(ctx context.Context, variableID resource.ID) (*variable.VariableSet, error)
	CreateVariableSetVariable(ctx context.Context, setID resource.ID, opts variable.CreateVariableOptions) (*variable.Variable, error)
	UpdateVariableSetVariable(ctx context.Context, variableID resource.ID, opts variable.UpdateVariableOptions) (*variable.VariableSet, error)
	DeleteVariableSetVariable(ctx context.Context, variableID resource.ID) (*variable.VariableSet, error)
...
// templates holds dependencies needed by templ templates.
...
func NewHandlers(variables Client, authorizer authz.Interface) *Handlers {
	return &Handlers{
		client:     variables,
		authorizer: authorizer,
		templates:  &templates{Interface: authorizer},
	}
}
...
func (h *Handlers) createWorkspaceVariable(w http.ResponseWriter, r *http.Request) {
	...
	variable, err := h.client.CreateWorkspaceVariable(r.Context(), params.WorkspaceID, variable.CreateVariableOptions{
		Key:         params.Key,
		Value:       params.Value,
...
func (h *Handlers) listWorkspaceVariables(w http.ResponseWriter, r *http.Request) {
	...
	sets, err := h.client.ListWorkspaceVariableSets(r.Context(), workspaceID)
	if err != nil {
		helpers.Error(r, w, err.Error())
		return
	}
...
func (h *Handlers) editWorkspaceVariable(w http.ResponseWriter, r *http.Request) {
	...
	wv, err := h.client.GetWorkspaceVariable(r.Context(), variableID)
	if err != nil {
		helpers.Error(r, w, err.Error())
		return
	}
	ws, err := h.client.GetWorkspace(r.Context(), wv.WorkspaceID)
	if err != nil {
		helpers.Error(r, w, err.Error())
		return
	}
...
func (h *Handlers) updateWorkspaceVariable(w http.ResponseWriter, r *http.Request) {
	...
	wv, err := h.client.UpdateWorkspaceVariable(r.Context(), params.VariableID, variable.UpdateVariableOptions{
		Key:         params.Key,
		Value:       params.Value,
		Description: params.Description,
		Category:    params.Category,
...
func (h *Handlers) deleteWorkspaceVariable(w http.ResponseWriter, r *http.Request) {
	...
	wv, err := h.client.DeleteWorkspaceVariable(r.Context(), variableID)
	if err != nil {
		helpers.Error(r, w, err.Error())
		return
	}
...
func (h *Handlers) listVariableSets(w http.ResponseWriter, r *http.Request) {
	...
	sets, err := h.client.ListVariableSets(r.Context(), params.Organization)
	if err != nil {
		helpers.Error(r, w, err.Error())
		return
	}
...
func (h *Handlers) createVariableSet(w http.ResponseWriter, r *http.Request) {
	...
	set, err := h.client.CreateVariableSet(r.Context(), params.Organization, variable.CreateVariableSetOptions{
		Name:        *params.Name,
		Description: params.Description,
...
func (h *Handlers) editVariableSet(w http.ResponseWriter, r *http.Request) {
	...
	set, err := h.client.GetVariableSet(r.Context(), setID)
	if err != nil {
		helpers.Error(r, w, err.Error())
		return
	}
...
func (h *Handlers) updateVariableSet(w http.ResponseWriter, r *http.Request) {
	...
	set, err := h.client.UpdateVariableSet(r.Context(), params.SetID, variable.UpdateVariableSetOptions{
		Name:        params.Name,
		Description: params.Description,
		Global:      params.Global,
		Workspaces:  workspaceIDs,
...
func (h *Handlers) deleteVariableSet(w http.ResponseWriter, r *http.Request) {
	...
	set, err := h.client.DeleteVariableSet(r.Context(), setID)
	if err != nil {
		helpers.Error(r, w, err.Error())
		return
	}
...
func (h *Handlers) createVariableSetVariable(w http.ResponseWriter, r *http.Request) {
	...
	variable, err := h.client.CreateVariableSetVariable(r.Context(), params.SetID, variable.CreateVariableOptions{
		Key:         params.Key,
		Value:       params.Value,
		Description: params.Description,
		Category:    params.Category,
...
func (h *Handlers) editVariableSetVariable(w http.ResponseWriter, r *http.Request) {
	...
	set, err := h.client.GetVariableSetByVariableID(r.Context(), variableID)
	if err != nil {
		helpers.Error(r, w, err.Error())
		return
	}
...
func (h *Handlers) updateVariableSetVariable(w http.ResponseWriter, r *http.Request) {
	...
	set, err := h.client.UpdateVariableSetVariable(r.Context(), params.VariableID, variable.UpdateVariableOptions{
		Key:         params.Key,
		Value:       params.Value,
		Description: params.Description,
		Category:    params.Category,
...
func (h *Handlers) deleteVariableSetVariable(w http.ResponseWriter, r *http.Request) {
	...
	set, err := h.client.DeleteVariableSetVariable(r.Context(), variableID)
	if err != nil {
		helpers.Error(r, w, err.Error())
		return
	}
...
func (h *Handlers) getAvailableWorkspaces(ctx context.Context, org organization.Name) ([]resource.Info, error) {
	...
	workspaces, err := resource.ListAll(func(opts resource.PageOptions) (*resource.Page[*workspace.Workspace], error) {
		return h.client.ListWorkspaces(ctx, workspace.ListOptions{
			Organization: &org,
			PageOptions:  opts,
		})
	})
