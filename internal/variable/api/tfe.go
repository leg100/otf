...
type tfeClient interface {
	...
	CreateVariable(ctx context.Context, workspaceID resource.TfeID, opts variable.CreateVariableOptions) (*variable.Variable, error)
...
func (a *TFEAPI) createVariable(w http.ResponseWriter, r *http.Request) {
	...
	var opts variable.TFEVariableCreateOptions
	if err := tfeapi.Unmarshal(r.Body, &opts); err != nil {
		tfeapi.Error(w, err)
		return
	}
	v, err := a.Client.CreateVariable(r.Context(), parentID, variable.CreateVariableOptions{
		Key:         opts.Key,
		Value:       opts.Value,
...
func (a *TFEAPI) listVariables(w http.ResponseWriter, r *http.Request) {
	...
	to := make([]*variable.TFEVariable, len(variables))
...
func (a *TFEAPI) updateVariable(w http.ResponseWriter, r *http.Request) {
	...
	var opts variable.TFEVariableUpdateOptions
	if err := tfeapi.Unmarshal(r.Body, &opts); err != nil {
		tfeapi.Error(w, err)
		return
	}

	v, err := a.Client.UpdateVariable(r.Context(), variableID, variable.UpdateVariableOptions{
		Key:         opts.Key,
		Value:       opts.Value,
		Description: opts.Description,
		Category:    (*variable.VariableCategory)(opts.Category),
...
func (a *TFEAPI) createVariableSet(w http.ResponseWriter, r *http.Request) {
	...
	var params variable.TFEVariableSetCreateOptions
	if err := tfeapi.Unmarshal(r.Body, &params); err != nil {
		tfeapi.Error(w, err)
		return
	}
	set, err := a.Client.CreateVariableSet(r.Context(), pathParams.Organization, variable.CreateVariableSetOptions{
		Name:        params.Name,
...
func (a *TFEAPI) updateVariableSet(w http.ResponseWriter, r *http.Request) {
	...
	var params variable.TFEVariableSetUpdateOptions
	if err := tfeapi.Unmarshal(r.Body, &params); err != nil {
		tfeapi.Error(w, err)
		return
	}
	set, err := a.Client.UpdateVariableSet(r.Context(), setID, variable.UpdateVariableSetOptions{
		Name:        params.Name,
		Description: params.Description,
		Global:      params.Global,
	})
...
func (a *TFEAPI) listVariableSets(w http.ResponseWriter, r *http.Request) {
	...
	to := make([]*variable.TFEVariableSet, len(sets))
...
func (a *TFEAPI) applySetToWorkspaces(w http.ResponseWriter, r *http.Request) {
	...
	var params []*variable.TFEWorkspace
	if err := tfeapi.Unmarshal(r.Body, &params); err != nil {
...
func (a *TFEAPI) convertVariableSet(from *variable.VariableSet) *variable.TFEVariableSet {
	to := &variable.TFEVariableSet{
		ID:          from.ID,
		Name:        from.Name,
	...
	to.Variables = make([]*variable.TFEVariable, len(from.Variables))
...
func (a *TFEAPI) convertVariable(from *variable.Variable) *variable.TFEVariable {
	to := &variable.TFEVariable{
		ID:          from.ID,
		Key:         from.Key,
		Value:       from.Value,
		Description: from.Description,
	...
	case resource.VariableSetKind:
		to.VariableSet = &variable.TFEVariableSet{ID: from.ParentID}
	}
	return to
}
...
func variableError(w http.ResponseWriter, err error) {
	if internal.ErrorIs(err,
		variable.ErrVariableDescriptionMaxExceeded,
		variable.ErrVariableKeyMaxExceeded,
		variable.ErrVariableValueMaxExceeded,
	) {
		tfeapi.Error(w, err, tfeapi.WithStatus(http.StatusUnprocessableEntity))
		return
	}
