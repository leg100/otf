...
func TestGetDefaultBranch(t *testing.T) {
	...
	client := newTestServerClient(t,
		WithRepo(vcs.Repo{Owner: "acme", Name: "test"}),
...
func TestListRepositories(t *testing.T) {
	...
	assert.Equal(t, []vcs.Repo{{Owner: "acme", Name: "test"}}, got)
}
...
func TestGetRepoTarball(t *testing.T) {
	...
	got, gotref, err := client.GetRepoTarball(ctx, vcs.GetRepoTarballOptions{
		Repo: vcs.Repo{Owner: "acme", Name: "test"},
	})
	require.NoError(t, err)
	...
	got, gotref, err = client.GetRepoTarball(ctx, vcs.GetRepoTarballOptions{
		Repo: vcs.Repo{Owner: "acme", Name: "nonexistent-repo"},
	})
	require.Error(t, err)
	require.Zero(t, got)
	...
	got, gotref, err = client.GetRepoTarball(ctx, vcs.GetRepoTarballOptions{
		Repo: vcs.Repo{Owner: "nonexisting-org", Name: "test"},
	})
	require.Error(t, err)
	require.Zero(t, got)
	require.Zero(t, gotref)
...
func TestListPullRequestFiles(t *testing.T) {
	...
	files, err := client.ListPullRequestFiles(ctx, vcs.Repo{Owner: "acme", Name: "test"}, 432)
	require.NoError(t, err)
	// archive returned and non-empty
	...
	_, err = client.ListPullRequestFiles(ctx, vcs.Repo{Owner: "acme", Name: "test"}, 234)
	require.Error(t, err)
}
...
func TestGetCommit(t *testing.T) {
	...
	got, err := client.GetCommit(ctx, vcs.Repo{Owner: "acme", Name: "test"}, "")
	require.NoError(t, err)
	...
	got, err = client.GetCommit(ctx, vcs.Repo{Owner: "acme", Name: "test"}, "main")
	require.NoError(t, err)
	...
	got, err = client.GetCommit(ctx, vcs.Repo{Owner: "acme", Name: "test"}, "refs/heads/main")
	require.NoError(t, err)
	...
	got, err = client.GetCommit(ctx, vcs.Repo{Owner: "acme", Name: "test"}, "v0.0.1")
	require.NoError(t, err)

	assert.Equal(t, sha, got.SHA)
	assert.Equal(t, "user", got.Author.Username)

	got, err = client.GetCommit(ctx, vcs.Repo{Owner: "acme", Name: "nonexistent-repo"}, "")
	require.Error(t, err)
	require.Zero(t, got)

	got, err = client.GetCommit(ctx, vcs.Repo{Owner: "nonexistent-org", Name: "test"}, "")
	require.Error(t, err)
	require.Zero(t, got)
}
