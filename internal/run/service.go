...
// EnqueueRunPlan enqueues a plan for the run.
func (s *Service) EnqueueRunPlan(ctx context.Context, runID resource.TfeID) (run *Run, err error) {
...
func (s *Service) AfterEnqueueRunPlan(hook func(context.Context, *Run) error) {
...
// StartRunPhase starts a run phase.
func (s *Service) StartRunPhase(ctx context.Context, runID resource.TfeID, phase PhaseType, _ PhaseStartOptions) (*Run, error) {
	run, err := s.db.UpdateStatus(ctx, runID, func(ctx context.Context, run *Run) error {
		return run.Start()
	})
	if err != nil {
...
// FinishRunPhase finishes a phase. Creates a report of changes before updating the status of
// the run.
func (s *Service) FinishRunPhase(ctx context.Context, runID resource.TfeID, phase PhaseType, opts PhaseFinishOptions) (*Run, error) {
...
func (s *Service) AfterEnqueueRunApply(hook func(context.Context, *Run) error) {
...
func (s *Service) createApplyReport(ctx context.Context, runID resource.TfeID) (Report, error) {
	logs, err := s.GetRunChunk(ctx, GetChunkOptions{
		RunID: runID,
		Phase: ApplyPhase,
	})
	if err != nil {
...
// GetRunChunk retrieves a chunk of logs for a run phase.
func (s *Service) GetRunChunk(ctx context.Context, opts GetChunkOptions) (Chunk, error) {
...
// PutRunChunk writes a chunk of logs for a run phase
func (s *Service) PutRunChunk(ctx context.Context, opts PutChunkOptions) error {
