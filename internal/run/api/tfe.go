...
type tfeClient interface {
	...
	GetRunPlanFile(ctx context.Context, id resource.TfeID, format run.PlanFormat) ([]byte, error)
...
func (a *tfe) createRun(w http.ResponseWriter, r *http.Request) {
	var params run.TFERunCreateOptions
...
func (a *tfe) listRuns(w http.ResponseWriter, r *http.Request) {
	var params run.TFERunListOptions
	...
	var planOnly *bool
	if slices.Contains(operations, string(run.RunOperationPlanOnly)) {
		planOnly = new(true)
	}
	...
	if slices.Contains(operations, string(run.RunOperationSavePlan)) {
		a.RespondWithPage(w, r, []*run.TFERun{}, &resource.Pagination{})
		return
	}
...
func (a *tfe) listRunsWithOptions(w http.ResponseWriter, r *http.Request, opts run.ListOptions) {
	...
	// convert items
	items := make([]*run.TFERun, len(page.Items))
...
func (a *tfe) cancelRun(w http.ResponseWriter, r *http.Request) {
	...
	if err = a.client.CancelRun(r.Context(), id); err != nil {
		if internal.ErrorIs(err, run.ErrRunCancelNotAllowed, run.ErrRunForceCancelNotAllowed) {
...
func (a *tfe) getPlanJSON(w http.ResponseWriter, r *http.Request) {
	...
	// otf's plan IDs are simply the corresponding run ID
	json, err := a.client.GetRunPlanFile(r.Context(), resource.ConvertTfeID(id, "run"), run.PlanFormatJSON)
	if err != nil {
		tfeapi.Error(w, err)
		return
	}
...
func (a *tfe) listRunEvents(w http.ResponseWriter, r *http.Request) {
	a.Responder.Respond(w, r, []*run.TFERunEvent{}, http.StatusOK)
}
...
func (a *tfe) toRun(from *run.Run, ctx context.Context) (*run.TFERun, error) {
	accessRequest := &authz.Request{ID: &from.ID}
	perms := &run.TFERunPermissions{
	...
	var timestamps run.TFERunStatusTimestamps
	...
	to := &run.TFERun{
		...
		// Relations
		Plan:  &run.TFEPlan{ID: resource.ConvertTfeID(from.ID, "plan")},
		Apply: &run.TFEApply{ID: resource.ConvertTfeID(from.ID, "apply")},
		...
		Workspace: &run.TFEWorkspace{ID: from.WorkspaceID},
	}
	to.Variables = make([]run.TFERunVariable, len(from.Variables))
	for i, from := range from.Variables {
		to.Variables[i] = run.TFERunVariable(from)
	}
	...
	if timestamps.CanceledAt != nil {
		...
		cooledOff := timestamps.CanceledAt.Add(run.ForceCancelCoolOff)
		to.ForceCancelAvailableAt = &cooledOff
	} else if from.CancelSignaledAt != nil {
		// run not successfully canceled yet
		cooledOff := from.CancelSignaledAt.Add(run.ForceCancelCoolOff)
		to.ForceCancelAvailableAt = &cooledOff
	}
	return to, nil
}

func (a *tfe) toPlan(plan run.Phase, r *http.Request) (*run.TFEPlan, error) {
...
func (a *tfe) toApply(apply run.Phase, r *http.Request) (*run.TFEApply, error) {
...
func (a *tfe) toResourceReport(from *run.Report) run.TFEResourceReport {
	var to run.TFEResourceReport
	if from != nil {
...
func (a *tfe) toPhaseTimestamps(from []run.PhaseStatusTimestamp) *run.TFEPhaseStatusTimestamps {
	var timestamps run.TFEPhaseStatusTimestamps
	for _, ts := range from {
		switch ts.Status {
...
func (a *tfe) logURL(r *http.Request, phase run.Phase) (string, error) {
