...
func TestTail(t *testing.T) {
	t.Run("receive chunk event", func(t *testing.T) {
		...
		stream, err := app.Tail(t.Context(), TailOptions{
	...
	t.Run("receive existing chunk", func(t *testing.T) {
		...
		stream, err := tailer.Tail(t.Context(), TailOptions{
			RunID: testutils.ParseID(t, "run-123"),
			Phase: PlanPhase,
		})
		require.NoError(t, err)
	...
	t.Run("receive existing chunk and overlapping published chunk", func(t *testing.T) {
		...
		stream, err := svc.Tail(t.Context(), TailOptions{
			RunID: testutils.ParseID(t, "run-123"),
			Phase: PlanPhase,
		})
		require.NoError(t, err)
...
func (f *fakeSubService) Subscribe(ctx context.Context) (<-chan pubsub.Event[Chunk], func(), error) {
	go func() {
		<-ctx.Done()
		close(f.stream)
	}()
