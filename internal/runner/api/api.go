...
type (
	...
	apiClient interface {
		...
		GetJob(ctx context.Context, jobID resource.TfeID) (*runner.Job, error)
		StartJob(ctx context.Context, jobID resource.TfeID) ([]byte, error)
		AwaitJobSignal(ctx context.Context, jobID resource.TfeID) func() (runner.JobSignal, error)
		FinishJob(ctx context.Context, jobID resource.TfeID, opts runner.FinishJobOptions) error
...
func (a *API) registerAgent(w http.ResponseWriter, r *http.Request) {
	var opts runner.RegisterRunnerOptions
...
func (a *API) awaitAllocatedJobs(w http.ResponseWriter, r *http.Request) {
	...
	runner, err := runner.RunnerFromContext(r.Context())
	if err != nil {
...
func (a *API) updateAgentStatus(w http.ResponseWriter, r *http.Request) {
	// retrieve rnr, which contains ID of calling agent
	rnr, err := runner.RunnerFromContext(r.Context())
	if err != nil {
		http.Error(w, err.Error(), http.StatusUnauthorized)
		return
	}
	...
	err = a.Client.UpdateStatus(r.Context(), rnr.ID, params.Status)
	if err != nil {
		if errors.Is(err, runner.ErrInvalidStateTransition) {
			tfeapi.Error(w, err)
		} else {
			http.Error(w, err.Error(), http.StatusInternalServerError)
		}
