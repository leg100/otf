...
func (q *DBQuerier) FindVariables(ctx context.Context, workspaceID pgtype.Text) ([]FindVariablesRow, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&item.VariableID, &item.Key, &item.Value, &item.Description, &item.Category, &item.Sensitive, &item.HCL, &item.WorkspaceID, &item.VersionID); err != nil {
...
func (q *DBQuerier) FindVariable(ctx context.Context, variableID pgtype.Text) (FindVariableRow, error) {
	...
	if err := row.Scan(&item.VariableID, &item.Key, &item.Value, &item.Description, &item.Category, &item.Sensitive, &item.HCL, &item.WorkspaceID, &item.VersionID); err != nil {
