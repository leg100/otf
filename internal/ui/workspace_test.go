...
func TestWorkspace_Create(t *testing.T) {
	...
	h := &workspaceHandlers{
		client: &workspace.FakeService{Workspaces: []*workspace.Workspace{ws}},
	}
...
func TestGetWorkspaceHandler(t *testing.T) {
	var (
		...
		unlocked = workspace.NewTestWorkspace(t, nil)
		locked   = workspace.NewTestWorkspace(t, nil)
	)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			app := &workspaceHandlers{
				uiHelpers: &uiHelpers{
					authorizer: authz.NewAllowAllAuthorizer(),
				},
				authorizer: authz.NewAllowAllAuthorizer(),
				client:     &workspace.FakeService{Workspaces: []*workspace.Workspace{tt.workspace}},
			}
...
func TestEditWorkspaceHandler(t *testing.T) {
	...
	wsConnected := workspace.NewTestWorkspace(t, &workspace.CreateOptions{
	...
	tests := []struct {
		...
		},
		{
			...
			user: user.SiteAdmin,
			policy: workspace.Policy{
				Permissions: []workspace.Permission{
...
func TestUpdateWorkspaceHandler(t *testing.T) {
	...
	ws := &workspace.Workspace{ID: testutils.ParseID(t, "ws-123"), Organization: org1}
	app := &workspaceHandlers{
		client: &workspace.FakeService{Workspaces: []*workspace.Workspace{ws}},
	}
...
func TestListWorkspacesHandler(t *testing.T) {
	...
	for i := 1; i <= n; i++ {
		workspaces[i-1] = &workspace.Workspace{ID: testutils.ParseID(t, fmt.Sprintf("ws-%d", i))}
	}
	app := &workspaceHandlers{
		authorizer: authz.NewAllowAllAuthorizer(),
		client:     &workspace.FakeService{Workspaces: workspaces},
	}
...
func TestListWorkspacesHandler_WithLatestRun(t *testing.T) {
	ws := &workspace.Workspace{ID: testutils.ParseID(t, "ws-foo"), LatestRun: &workspace.LatestRun{Status: "applied", ID: testutils.ParseID(t, "run-123")}}
	app := &workspaceHandlers{
		authorizer: authz.NewAllowAllAuthorizer(),
		client:     &workspace.FakeService{Workspaces: []*workspace.Workspace{ws}},
	}
...
func TestFilterUnassigned(t *testing.T) {
	policy := workspace.Policy{Permissions: []workspace.Permission{
